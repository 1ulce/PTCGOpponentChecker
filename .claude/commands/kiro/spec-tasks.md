...
1. `docs/.kiro/specs/$1/requirements.md` - Feature requirements (EARS format)
2. `docs/.kiro/specs/$1/design.md` - Technical design document
3. `docs/.kiro/steering/` - Project-wide guidelines and constraints:
   - **Core files (always load)**:
     - @docs/.kiro/steering/product.md - Business context, product vision, user needs
     - @docs/.kiro/steering/tech.md - Technology stack, frameworks, libraries
     - @docs/.kiro/steering/structure.md - File organization, naming conventions, code patterns
   - **Custom steering files** (load all EXCEPT "Manual" mode in `AGENTS.md`):
     - Any additional `*.md` files in `docs/.kiro/steering/` directory
     - Examples: `api.md`, `testing.md`, `security.md`, etc.
   - (Task planning benefits from comprehensive context)
4. `docs/.kiro/specs/$1/tasks.md` - Existing tasks (only if merge mode)
...
### Document Generation
- Generate `docs/.kiro/specs/$1/tasks.md` using the exact numbering format above
...
- **Task descriptions**: Use natural language for "what to do" (implementation details in design.md)
 - Update `docs/.kiro/specs/$1/spec.json`:
