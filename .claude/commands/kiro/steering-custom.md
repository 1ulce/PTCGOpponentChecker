...
Create custom steering documents in `docs/.kiro/steering/` for specialized contexts beyond the three foundational files (product.md, tech.md, structure.md).

## Current Steering Status

### Existing Steering Documents
- Core steering files: !`ls -la docs/.kiro/steering/*.md 2>/dev/null || echo "No steering directory found"`
- Custom steering count: !`if [ -d "docs/.kiro/steering" ]; then count=0; for f in docs/.kiro/steering/*.md; do if [ -f "$f" ] && [ "$f" != "docs/.kiro/steering/product.md" ] && [ "$f" != "docs/.kiro/steering/tech.md" ] && [ "$f" != "docs/.kiro/steering/structure.md" ]; then count=$((count + 1)); fi; done; echo "$count"; else echo "0"; fi`
...
2. **Create the document** in `docs/.kiro/steering/` with:
